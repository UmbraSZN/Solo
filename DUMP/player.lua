...
player.parryStun = 0.4
player.blockExtend = 0
...
function player:update(dt)
    ...
    -- elseif player.state == "blocking" then
    --     if player.blockExtend > 0 then
    --         player.blockExtend = player.blockExtend - dt
    --         if player.blockExtend <= 0 then
    ...
    if player.state == "attacking" then
        ...
        if player.attackTimer >= player.animTimer then
            ...
            for _, e in ipairs(query) do
                ...
                e:hit(10, player.lightStun, vx, vy) --change damage value based on a variable later
        ...
        --BLOCKING AND PARRYS IS FUKCED
    else 
        player.attackTimer = 0

    end

    if player.state == "blocking" and player.blockExtend then
        player.blockExtend = 0.25
        player.blockExtend = player.blockExtend - dt
        if player.blockExtend <= 0 then
            player.blockExtend = false
            player.blockDuration = 0
            player.state = "default"
        end
    end
...
function player:hit(dmg, stun, vx, vy)
    ...
    if self.iFrames <= 0 then
        ...
        if self.blockDuration < 0.35 and self.state == "blocking" then
            --parried (no damage + counter stun)
            print("parried")
            return self.parryStun
